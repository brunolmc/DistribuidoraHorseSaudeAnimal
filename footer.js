...
function Footer({ setRoute }) {
  ...
  const cols = [
    ...
    ['Atendimento', [['Fale conosco', nav('contact')], ['Seja um cliente', nav('contact')], ['Marcas parceiras', nav('home')]]],
  ];
  const social = [
  ...
  return (
    <footer style={{ background: 'var(--ink-700)', color: 'var(--paper-200)', padding: 'var(--space-8) var(--gutter) var(--space-6)' }}>
      <div style={{ maxWidth: 'var(--container)', margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '1.4fr repeat(3,1fr)', gap: isMobile ? 28 : 40 }}>
          <div style={{ gridColumn: isMobile ? '1 / -1' : 'auto' }}>
            <Logo variant="mark" tone="light" height={56} base="./" />
        ...
        <div style={{ marginTop: 44, paddingTop: 22, borderTop: '1px solid rgba(255,255,255,0.12)', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, fontFamily: 'var(--font-sans)', fontSize: 12.5, color: 'var(--ink-300)' }}>
